...
impl Repository {
    pub async fn add_other_quotes(
        &self,
        opportunity_id: Uuid,
        jupiter_ultra_client: Option<JupiterUltraSwapApiClient>,
        quote_create: QuoteCreate,
    ) -> Result<(), RestError> {
        let other_quotes = self
            .get_other_quotes(jupiter_ultra_client, quote_create)
            .await;
        if !other_quotes.is_empty() {
            self.db
                .add_other_quotes(opportunity_id, other_quotes)
                .await?;
        }
        Ok(())
    }
    ...
    async fn get_other_quotes(
        ...
        if let Some(jupiter_ultra_client) = jupiter_ultra_client {
            let limiter = self
