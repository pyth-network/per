...
contract VaultScript is Script {
    ...
    function setUpVault(uint256 qT1, uint256 qT2, bool collatT1) public {
        ...
        if (collatT1) {
            ...
            numVaults = TokenVault(payable(tokenVaultLatest)).createVault(token1Latest, token2Latest, qT1, qT2, 110 * (10**16), 20 * (10**16), idToken1Latest, idToken2Latest);
            vm.stopBroadcast();
        } else {
            vm.startBroadcast(depositorSkLatest);
            IERC20(token2Latest).approve(address(tokenVaultLatest), qT2);
            numVaults = TokenVault(payable(tokenVaultLatest)).createVault(token2Latest, token1Latest, qT2, qT1, 110 * (10**16), 20 * (10**16), idToken1Latest, idToken2Latest);
            vm.stopBroadcast();
        }
