...
export const useExpressRelayClientContext = (endpoint: string) => {
  const expressRelayClient = useMemo(() => new Client({baseUrl: endpoint}), [endpoint])
...
export const useExpressRelayClient = () => {
  ...
  if (expressRelayClient === undefined) {
    throw new Error("This component must be wrapped in an ExpressRelayProvider")
  }
  return expressRelayClient
}

export const ExpressRelayProvider = ({
  endpoint,
  ...props
}: Props) => {
  const state = useExpressRelayClientContext(endpoint)
  return <ExpressRelayClientContext.Provider value={state}>{props.children}</ExpressRelayClientContext.Provider>
}
