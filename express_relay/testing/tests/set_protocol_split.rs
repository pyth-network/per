...
use testing::{express_relay::{helpers::get_router_config, set_router_split::get_set_router_split_instruction}, helpers::{assert_custom_error, generate_and_fund_key, submit_transaction}, setup::{setup, SetupParams}};
...
fn test_set_router_split() {
    ...
    let set_router_split_ix = get_set_router_split_instruction(&admin, router, split_router);
    submit_transaction(&mut svm, &[set_router_split_ix], &admin, &[&admin]).expect("Transaction failed unexpectedly");
...
fn test_set_router_split_fail_wrong_admin() {
    ...
    let router = Keypair::new().pubkey();
    let split_router: u64 = 5000;
    let set_router_split_ix = get_set_router_split_instruction(&wrong_admin, router, split_router);
    let tx_result = submit_transaction(&mut svm, &[set_router_split_ix], &wrong_admin, &[&wrong_admin]).expect_err("Transaction should have failed");
...
fn test_set_router_split_fail_high_split() {
    ...
    let set_router_split_ix = get_set_router_split_instruction(&admin, router, split_router);
    let tx_result = submit_transaction(&mut svm, &[set_router_split_ix], &admin, &[&admin]).expect_err("Transaction should have failed");
