...
use express_relay::state::SEED_METADATA;
...
async fn test_withdraw_fees() {
    ...
    let transfer_ix = transfer(&payer.pubkey(), &express_relay_metadata, 1_000_000_000);
    ...
    println!("balance_express_relay_metadata: {}", balance_express_relay_metadata);
    println!("balance_admin: {}", balance_admin);
}
