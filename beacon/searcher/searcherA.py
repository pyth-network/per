...
def create_liquidation_intent(
    ...
    repay_tokens = [(opp['repay_tokens'][0][0], int(opp['repay_tokens'][0][1],16))]
    receipt_tokens = [(opp['receipt_tokens'][0][0], int(opp['receipt_tokens'][0][1],16))]
    ...
    signature_liquidator = construct_signature_liquidator(repay_tokens, receipt_tokens, opp['contract'], liq_calldata, bid, valid_until, sk_liquidator)
    ...
    calldata = LIQUIDATION_ADAPTER_FN_SIGNATURE + encode([LIQUIDATION_ADAPTER_CALLDATA_TYPES],[tuple(liquidation_adapter_calldata.values())]).hex()
...
async def main():
    ...
    intent = create_liquidation_intent(liquidatable[0], sk_liquidator, VALID_UNTIL, BID)
